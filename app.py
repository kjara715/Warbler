...
def profile():
    ...
    #check to see if this is the user logged in
    if not g.user:
        flash("Access unauthorized.", "danger")
        return redirect("/")
    ...
    if form.validate_on_submit():
        try:
            ...
            g.user.username=form.username.data #must be unique
            g.user.email=form.email.data #also must be unique...
...
def like_message(message_id):
    ...
    #want logic so that if the button is hit again, the message is unliked.
...
def homepage():
    ...
    if g.user:
        ...
        #get all the likes with g.user.id=user_id and get the message_id's from this
        likes=Likes.query.with_entities(Likes.message_id).filter(Likes.user_id == g.user.id).all()
        ...
        return render_template('home.html', messages=messages, likes=likes, result=result)
